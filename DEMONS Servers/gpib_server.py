    ...
    def refreshDevices(self):
        ...
        try:
            ...
            for addr in additions:
                try:
                    if not addr.startswith(KNOWN_DEVICE_TYPES) or not addr.endswith(KNOWN_TERM):
                        continue
                    ...
                    if addr.endswith('SOCKET'):
                        instr.write_termination = '\n'
                        instr.timeout = 1000
                        instr.set_visa_attribute(visa.constants.VI_ATTR_SUPPRESS_END_EN,visa.constants.VI_FALSE)
